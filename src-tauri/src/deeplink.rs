...
pub enum NavRoute {
    Workspace { id: String },
    AgentSpawn { agent_type: String, workspace_id: Option<String>, cwd: Option<String> },
    Task { id: String },
    Memory { slug: String },
    Chat { text: String },
    Unknown { url: String },
...
pub fn parse(url: &str) -> NavRoute {
    let parsed = match Url::parse(url) {
        Ok(u) => u,
        Err(_) => return NavRoute::Unknown { url: url.to_string() },
    };
    if parsed.scheme() != "pigide" {
        return NavRoute::Unknown { url: url.to_string() };
    }
    ...
    match host.as_str() {
        "workspace" => path
            .first()
            .map(|id| NavRoute::Workspace { id: (*id).to_string() })
            .unwrap_or_else(|| NavRoute::Unknown { url: url.to_string() }),
        ...
        "task" => path
            .first()
            .map(|id| NavRoute::Task { id: (*id).to_string() })
            .unwrap_or_else(|| NavRoute::Unknown { url: url.to_string() }),
        "memory" => path
            .first()
            .map(|s| NavRoute::Memory { slug: (*s).to_string() })
            .unwrap_or_else(|| NavRoute::Unknown { url: url.to_string() }),
        "chat" => NavRoute::Chat {
            text: q.get("text").cloned().unwrap_or_default(),
        },
        _ => NavRoute::Unknown { url: url.to_string() },
    }
}
...
mod tests {
    ...
    fn parses_agent_spawn_with_query() {
        ...
        match r {
            NavRoute::AgentSpawn { agent_type, workspace_id, cwd } => {
